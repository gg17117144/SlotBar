System.register(["__unresolved_0", "cc"], function (_export, _context) {
  ...
  return {
    ...
    execute: function () {
      ...
      _export("ReelSlot", ReelSlot = (_dec = ccclass('ReelView'), _dec2 = property(Prefab), _dec3 = property(Node), _dec4 = property(Number), _dec(_class = (_class2 = class ReelSlot extends Component {
        constructor() {
          super(...arguments);
        ...
        initReel(symbols) {
          ...
          for (var i = 0; i < symbols.length; i++) {
            var item = instantiate(this.reelItemPrefab);
            var sprite = item.getComponentInChildren(Sprite);
          ...
          for (var index = 0; index < this.symbolDataList.length; index++) {
        ...
        updateVisibleSymbols() {
          var count = this.symbolNodes.length;
          var halfRange = (count - 1) / 2 * this.itemHeight; // 200

          var totalHeight = count * this.itemHeight; // 500

          for (var i = 0; i < count; i++) {
            var item = this.symbolNodes[i]; // 基準位置：中心對齊，從 200 到 -200

            var baseY = halfRange - i * this.itemHeight; // 200,100,0,-100,-200 (依照5張圖片為範例)
            // 讓圖片往下滾動

            var newY = baseY - this.currentOffset; // 位置循環維持在 [-halfRange, halfRange]
        ...
        spinToSymbol(symbolType, spinRounds, duration, onComplete) {
          if (duration === void 0) {
            duration = 0.5;
          }
          ...
          var targetIndex = this.symbolDataList.findIndex(s => s.type === symbolType);
          ...
          var currentIndex = this.currentSymbolIndex; // 計算步數

          var steps = (targetIndex - currentIndex + this.symbolDataList.length) % this.symbolDataList.length;
          if (steps === 0) steps = this.symbolDataList.length; // 計算加上亂數的總步數

          steps = spinRounds * this.symbolDataList.length + steps;
          var endOffset = this.currentOffset + steps * this.itemHeight; // 總時間 = 單次時間 * 步數
          ...
          tween({
            ...
            onComplete: () => {
              // 迴圈歸正偏移值
              var totalHeight = this.symbolDataList.length * this.itemHeight;
...
//# sourceMappingURL=eff71e3449332d49d4a21f0a24110fad2573b6cb.js.map